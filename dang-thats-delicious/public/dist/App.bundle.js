...
function autocomplete(input, latInput, lngInput) {
  if (!input) return; //skip fn from running if no input on homePage
...
function ajaxHeart(e) {
  ...
  _axios2.default.post(this.action).then(function (res) {
    var isHearted = _this.heart.classList.toggle('heart__button--hearted'); //accesses elements within heart form tag with name of heart
  ...
  _axios2.default.get('/api/stores/near?lat=' + lat + '&lng=' + lng).then(function (res) {
    ...
    markers.forEach(function (marker) {
      return marker.addListener('click', function () {
        // addListener is Google Maps equivalent of event Listener
...
function typeAhead(search) {
  ...
  searchInput.on('input', function () {
    ...
    // If no value, hide search results and return
    if (!this.value) {
